...
const port = forcePort ? +forcePort : (process.env.PORT ? +process.env.PORT : (thirdTour ? 8443 : 80));

const ariannaServer = process.env.ARIANNA_SERVER || 'http://localhost:8000/generate';
...
app.post('/arianna', async (req, res) => {
  try {
    const response = await fetch(ariannaServer, {
      method: 'POST',
